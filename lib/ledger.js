...
export async function applyTxToBalances(store, accounts = [], tx, sign = 1, rates) {
  ...
  if (tx.type === 'expense') {
    ...
    if (acc) await upd(acc, -sign * convert(amt, tx.currency, acc.currency, rates))
    ...
    if (from) await upd(from, -sign * convert(amt + fee, tx.currency, from.currency, rates))
    if (to) await upd(to, sign * convert(amt, tx.currency, to.currency, rates))
  }
}
