...
function supabaseStore(userId, userMeta = {}) {
  ...
  return {
    ...
    async createTransaction(t) {
      ...
      const { data, error } = await table('transactions').insert(fullPayload).select().single()
      if (!error && data) return { ...data, ...t }

      // If Postgres schema cache rejects columns like bill_id, description, or billing_month (PGRST204 / 42703),
      // retry with standard columns while embedding meta in items
      if (error && (error.code === 'PGRST204' || error.code === '42703' || /does not exist/i.test(error.message || ''))) {
        ...
        const { data: retryData, error: retryError } = await table('transactions').insert(safePayload).select().single()
        throwIf(retryError)
        return { ...retryData, ...t }
      }
      throwIf(error)
...
function localSubscriptions(scope) {
  ...
  return {
    ...
    async createSubscription(s) {
      const list = lsGet(key, []) || []
      const sub = { id: uuidv4(), created_at: new Date().toISOString(), show_as_bill: true, ...s }
      lsSet(key, [...list, sub])
      if (sub.show_as_bill) {
        ...
        const mirrorBill = {
          ...
          due_day: Math.min(31, Math.max(1, Number(sub.due_day) || 1)),
          category: sub.category || 'cat_bills',
          account_id: sub.account_id || null,
          auto_log_expense: !!sub.auto_log_expense,
        ...
        if (updated.show_as_bill) {
          if (existingIdx >= 0) {
            bills[existingIdx] = {
              ...
              due_day: Math.min(31, Math.max(1, Number(updated.due_day) || 1)),
              category: updated.category || 'cat_bills',
              account_id: updated.account_id || null,
              auto_log_expense: !!updated.auto_log_expense,
              is_subscription_mirror: true,
