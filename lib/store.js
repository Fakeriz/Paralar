    ...
    async createTransaction(t) {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST204' || error.code === '42703' || /does not exist/i.test(error.message || '')) {
          ...
          throwIf(minErr)
          const targetId = minData?.id || cleanPayload.id
          if (targetId) lsSet(`paralar_tx_storage_${targetId}`, storageProvider)
          return { ...cleanPayload, storage_provider: storageProvider, id: targetId }
        }
        throwIf(error)
      }
      const targetId = data?.id || cleanPayload.id
      if (targetId) lsSet(`paralar_tx_storage_${targetId}`, storageProvider)
      return { ...cleanPayload, storage_provider: storageProvider, id: targetId }
    },
