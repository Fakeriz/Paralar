...
export default function AccountsSheet({ open, onClose }) {
  ...
  return (
    <Sheet open={open} onClose={onClose} full title={t('accounts_cards')} right={<button type="button" onClick={() => { onClose?.(); setTimeout(() => openSheet('newAccount'), 120) }} className="h-8 w-8 rounded-full bg-zinc-950 text-white dark:bg-white dark:text-zinc-950 flex items-center justify-center transition-colors" aria-label="Add account"><Plus size={18} /></button>}>
      {accounts.length === 0 ? (
