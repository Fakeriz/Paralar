  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 pointer-events-none">
      <div className="mx-auto max-w-md pointer-events-auto">
        <div className="relative bg-background/90 backdrop-blur-xl border-t border-border/40 dark:border-white/5 safe-bottom">
            ...
            {items.map((it) => {
                ...
                return (
                  <div key="plus" className="flex justify-center">
                    <button
                      ...
                      aria-label="add"
                      data-testid="fab-add"
                      className="-translate-y-5 h-14 w-14 rounded-full bg-foreground text-background flex items-center justify-center shadow-xl shadow-black/25 active:scale-95 transition"
                    >
              ...
              return (
                <button key={it.id} type="button" onClick={() => onTab(it.id)} data-testid={`nav-${it.id}`} className="flex flex-col items-center justify-center gap-1 h-full pb-1">
                  <Icon size={22} strokeWidth={active ? 2.5 : 2} className={cn(active ? 'text-foreground' : 'text-muted-foreground')} />
                  <span className={cn('text-[10px] font-semibold', active ? 'text-foreground' : 'text-muted-foreground')}>{it.label}</span>
                </button>
              )
            })}
          </div>
